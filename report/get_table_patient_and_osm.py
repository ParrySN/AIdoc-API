...
def fetch_ai_predictions_patient_osm_table(cursor, channel, province, start, end):
...
def fetch_dentist_feedback(cursor, channel, province, start, end):
    start_date,end_date =du.check_date(cursor,start,end)
    query = """
...
def fetch_diagnosed_submission(cursor, channel, province,start, end):
    start_date,end_date =du.check_date(cursor,start,end)
    query = """
        SELECT 
        ai_prediction, 
